...
<details>
<summary>Change the VLM model</summary>
...
- Check [Hugging Face Model Hub](https://huggingface.co/models) for available VLM models
...
You can also check the [model cards on Jetson AI Lab](https://www.jetson-ai-lab.com/models.html) as a reference.
...
<details>
<summary>Change the Container Images</summary>
...
1. Download the new image with `docker pull` or build it by yourself
2. Find the service (`llm-server` or `riva-server`) you want to change in the `compose.yaml` file
3. Change the `image` attribute of the service to the new image
4. If you change the inference engine for `llm-server`, you may also need to change the `command` attribute
